...
let test_parse_simple_mapping_nat_not_equal_to_condition_is_true =
    ...
    let fails = add_fail(fails, EXPECT.to_be_true(Parser.parse("foo", "<>", 4n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_true(Parser.parse("foo", "<>", 2n, foo))) in
...
let test_parse_simple_mapping_nat_not_equal_to_condition_is_false =
    ...
    let fails = add_fail(fails, EXPECT.to_be_false(Parser.parse("foo", "<>", 3n, foo))) in
...
let test_parse_simple_mapping_nat_less_than_condition_is_true =
    ...
    let fails = add_fail(fails, EXPECT.to_be_true( Parser.parse("foo", "<", 4n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_true( Parser.parse("foo", "<", 7n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_true( Parser.parse("foo", "<", 5n, foo))) in
...
let test_parse_simple_mapping_nat_less_than_condition_is_false =
    ...
    let fails = add_fail(fails, EXPECT.to_be_false( Parser.parse("foo", "<", 3n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_false( Parser.parse("foo", "<", 2n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_false( Parser.parse("foo", "<", 1n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_false( Parser.parse("foo", "<", 0n, foo))) in
...
let test_parse_simple_mapping_nat_less_than_or_equal_condition_is_true =
    ...
    let fails = add_fail(fails, EXPECT.to_be_true( Parser.parse("foo", "<=", 3n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_true( Parser.parse("foo", "<=", 4n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_true( Parser.parse("foo", "<=", 7n, foo))) in
...
let test_parse_simple_mapping_nat_greater_than_condition_is_true =
    ...
    let fails = add_fail(fails, EXPECT.to_be_true( Parser.parse("foo", ">", 2n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_true( Parser.parse("foo", ">", 1n, foo))) in
...
let test_parse_simple_mapping_nat_greater_than_condition_is_false =
    ...
    let fails = add_fail(fails, EXPECT.to_be_false( Parser.parse("foo", ">", 3n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_false( Parser.parse("foo", ">", 4n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_false( Parser.parse("foo", ">", 5n, foo))) in
...
let test_parse_simple_mapping_nat_greater_than_or_equal_condition_is_false =
    ...
    let fails = add_fail(fails, EXPECT.to_be_false( Parser.parse("foo", ">=", 6n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_false( Parser.parse("foo", ">=", 5n, foo))) in
    let fails = add_fail(fails, EXPECT.to_be_false( Parser.parse("foo", ">=", 4n, foo))) in
